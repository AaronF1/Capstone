...
data["Total Number of Concessions Owned"] = data['Number of concession']
...
concession_min, concession_max = st.sidebar.slider(
    "Select Total Number of Concessions Owned Range:",
    min(data["Total Number of Concessions Owned"]),
    max(data["Total Number of Concessions Owned"]),
    (min(data["Total Number of Concessions Owned"]), max(data["Total Number of Concessions Owned"])),
    step=1,
)
...
filtered_data = filtered_data[(filtered_data['Total Number of Concessions Owned'] >= concession_min) & (filtered_data['Total Number of Concessions Owned'] <= concession_max)]
...
filtered_data['Total Number of Concessions Owned within Frequency Range'] = filtered_data.groupby('Owner')['Frequency GHZ rounded'].transform(lambda x: x.isin(selected_frequencies).sum())


filtered_data['Percentage of Concessions within Frequency Range relative To Total Owned'] = (filtered_data['Total Number of Concessions Owned within Frequency Range'] / filtered_data['Total Number of Concessions Owned'] * 100).round(2)
...
if (filtered_data2['Percentage of Concessions within Frequency Range relative To Total Owned'].max()-filtered_data2['Percentage of Concessions within Frequency Range relative To Total Owned'].min()) == 0:
    df_ranking_normalised['rank_percentage_of_concessions_within_frequency_range'] = 1
else:
    df_ranking_normalised['rank_percentage_of_concessions_within_frequency_range'] = (filtered_data2['Percentage of Concessions within Frequency Range relative To Total Owned'].values-filtered_data2['Percentage of Concessions within Frequency Range relative To Total Owned'].min())/(filtered_data2['Percentage of Concessions within Frequency Range relative To Total Owned'].max()-filtered_data2['Percentage of Concessions within Frequency Range relative To Total Owned'].min())
...
st.dataframe(filtered_data[['Reference', 'Owner', 'Municipality', 'Frequency GHZ rounded', 'Total Number of Concessions Owned', 'Total Number of Concessions Owned within Frequency Range', 'Percentage of Concessions within Frequency Range relative To Total Owned', 'investment_score', 'rank_average_time_until_termination', 'rank_average_days_since_last_opening', 'rank_percentage_of_concessions_within_frequency_range']])
